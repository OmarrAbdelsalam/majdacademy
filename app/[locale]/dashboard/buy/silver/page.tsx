...
import { getWallet, getCurrentPrices, buySilver } from "../../../../../lib/api";
...
export default function BuySilverPage() {
  ...
  const fetchData = async () => {
    ...
    if (pricesRes.success && pricesRes.data) {
      setCurrentPrice(pricesRes.data.silver_buy || 0);
    }
  };
  ...
  const tr = {
    title: isRTL ? "شراء فضة" : "Buy Silver",
    subtitle: isRTL ? "اشترِ فضة بالجرام من رصيدك" : "Buy silver by gram from your balance",
    weight: isRTL ? "الوزن (جرام)" : "Weight (grams)",
    balance: isRTL ? "الرصيد المتاح" : "Available balance",
    submit: isRTL ? "شراء فضة" : "Buy Silver",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const estimatedCost = Number(weight) * currentPrice;
    ...
    setLoading(true);
    const res = await buySilver({ weight }, lang);
    if (res.success) {
  ...
  return (
    <div>
      ...
      {/* Silver price card */}
      <div className="rounded-2xl p-6 mb-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #9CA3AF 0%, #D1D5DB 50%, #9CA3AF 100%)' }}>
        <div className="relative z-10 flex items-center justify-between">
          <div>
            <p className="text-[12px] font-bold text-white/70 uppercase tracking-wider mb-1">{tr.livePrice}</p>
            <p className="text-[32px] font-extrabold text-white leading-none">{currentPrice > 0 ? currentPrice.toLocaleString(undefined, {maximumFractionDigits: 0}) : "..."} <span className="text-[16px]">EGP/g</span></p>
          </div>
          <div className="w-14 h-14 rounded-full bg-white/15 flex items-center justify-center">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20.24 12.24a6 6 0 00-8.49-8.49L5 10.5V19h8.5z"/></svg>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#f0f0f0] p-8 max-w-lg">
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={loading} className="w-full py-4 font-bold text-[15px] rounded-2xl transition-all duration-200 shadow-[0_4px_20px_rgba(0,0,0,0.1)] hover:shadow-[0_6px_28px_rgba(0,0,0,0.15)] active:scale-[0.99]" style={{ background: 'linear-gradient(135deg, #9CA3AF 0%, #D1D5DB 100%)', color: '#1a1a1a' }}>
            {tr.submit}
          </button>
        </form>
      </div>
            ...
            <div className="p-8 flex flex-col items-center text-center">
              ...
              <div className="flex flex-col gap-3 w-full">
                <button 
                  ...
                  className="w-full py-3.5 bg-gradient-to-r from-[#9CA3AF] to-[#D1D5DB] text-[#1a1a1a] rounded-xl font-bold text-[15px] shadow-md hover:shadow-lg transition-all active:scale-[0.98]"
                >
