...
import { ChevronLeft, ChevronRight, CheckCircle2, Users, Star } from "lucide-react";
...
export default function KodlandHero3({ locale }: { locale: string }) {
  ...
  const countryText = isHome || activeCountry.id === "other" 
    ? (isRTL ? "بلدك" : "your country") 
    : (isRTL ? activeCountry.labelAr : activeCountry.labelEn);

  const t = {
    title1: isRTL ? "اصنع تفوق أبنائك" : "Shape Your Children's",
    title2: isRTL ? "في " : "Success in ",
    titleHighlight: isRTL ? "اللغة العربية" : "Arabic Language",
    desc1: isRTL ? "نقدم دروساً تفاعلية حية لتأسيس " : "We offer live interactive lessons to establish ",
    descHighlight: isRTL ? "وتفوق " : "and excel ",
    desc2: isRTL ? "أبنائكم في اللغة العربية والتربية الإسلامية بأسلوب يواكب المنهج المعتمد في " : "your children in Arabic and Islamic Education, aligning with the curriculum in ",
    desc3: isRTL ? "، بالإضافة إلى برامج مخصصة لتعليم لغير الناطقين بها" : ", plus specialized programs for non-native speakers",
    cta: isRTL ? "احجز حصة تجريبية مجانية" : "Book a Free Trial Class",
    stat1: isRTL ? "7 سنوات" : "7 Years",
    stat1desc: isRTL ? "خبرة تعليمية" : "Teaching Experience",
    stat2: isRTL ? "+500" : "500+",
    stat2desc: isRTL ? "طالب مستفيد" : "Benefiting Students",
    stat3: isRTL ? "4.9/5" : "4.9/5",
    stat3desc: isRTL ? "تقييم أولياء الأمور" : "Parents Rating",
  };
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 ${isRTL ? 'left-0' : 'right-0'} w-[80%] md:w-[50%] h-full opacity-[0.08] pointer-events-none select-none z-0 transform ${isRTL ? '' : 'scale-x-[-1]'}`}>
        <img 
          src="/hroof.png" 
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-12 relative z-10">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-8">
          ...
          >
            {/* Headlines */}
            <h1 className="text-[clamp(32px,8.5vw,48px)] sm:text-7xl lg:text-[80px] font-extrabold text-[#2a3042] leading-[1.2] sm:leading-[1.15] tracking-tight w-full drop-shadow-sm relative z-20">
              <span className={!isRTL ? "whitespace-nowrap" : ""}>
                {t.title1}
              </span>
            ...
            <p className="text-lg sm:text-xl lg:text-2xl text-[#262626]/80 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-medium px-2 sm:px-0">
              {t.desc1}<span className="hidden sm:inline">{t.descHighlight}</span>{t.desc2}{countryText}<span className="hidden sm:inline">{t.desc3}</span>.
            </p>
            ...
            <div className="pt-2 w-full sm:w-auto px-4 sm:px-0">
              ...
              >
                ...
                {t.cta}
                {!isRTL ? <ChevronRight className="w-5 h-5 sm:w-6 sm:h-6" /> : null}
              </button>
            </div>
            ...
            {/* Stats Row */}
            <div className={`flex flex-wrap justify-center lg:justify-start items-center gap-4 sm:gap-8 pt-8 sm:pt-10 w-full border-t border-gray-200/60 mt-4 ${isRTL ? '' : 'flex-row-reverse lg:flex-row'}`}>
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className={isRTL ? "text-right" : "text-left"}>
                  <p className="font-extrabold text-[#2a3042] text-lg sm:text-xl leading-tight">{t.stat1}</p>
                  <p className="text-xs sm:text-sm text-gray-500 font-medium">{t.stat1desc}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className={isRTL ? "text-right" : "text-left"}>
                  <p className="font-extrabold text-[#2a3042] text-lg sm:text-xl leading-tight">{t.stat2}</p>
                  <p className="text-xs sm:text-sm text-gray-500 font-medium">{t.stat2desc}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className={isRTL ? "text-right" : "text-left"}>
                  <p className="font-extrabold text-[#2a3042] text-lg sm:text-xl leading-tight">{t.stat3}</p>
                  <p className="text-xs sm:text-sm text-gray-500 font-medium">{t.stat3desc}</p>
                </div>
              </div>
